 ...
 * @author: Nianwei Liu
 * @fileoverview
 * <p>A TOC (Table of Contents) widget for ESRI ArcGIS Server JavaScript API. The namespace is <code>agd</code></p>
 */
// change log: 
...
define("agd/dijit/TOC", ['dojo/_base/declare', "dojo/has", "dojo/aspect", "dojo/_base/lang", "dojo/_base/array", "dojo/dom-construct", "dojo/dom-class", "dojo/dom-style", "dojo/dom-attr", 'dijit/_Widget', 'dijit/_Templated', 'dojo/Evented', 'dojox/gfx', 'dojo/fx', 'dojo/fx/Toggler', "esri/symbols/jsonUtils", "esri/geometry/scaleUtils", "esri/config", "esri/layers/ArcGISDynamicMapServiceLayer", "esri/layers/ArcGISTiledMapServiceLayer", "dojo/_base/sniff"], function(declare, has, aspect, lang, array, domConstruct, domClass, domStyle, domAttr, _Widget, _Templated, Evented, gfx, coreFx, Toggler, jsonUtils, scaleUtils, esriConfig, ArcGISDynamicMapServiceLayer, ArcGISTiledMapServiceLayer){
  ...
  var _TOCNode = declare([_Widget, _Templated], {
    templateString: '<div class="agdTOCNode">' +
    '<div data-dojo-attach-point="rowNode" data-dojo-attach-event="onclick:_onClick">' +
    '<span data-dojo-attach-point="contentNode" class="agdTOCContent">' +
    ...
    postCreate: function(){
      ...
      if (this.iconNode) {
        domClass.add(this.iconNode, 'agdTOCIcon');
      }
    ...
    _createRootLayerNode: function(rootLayer){
      domClass.add(this.rowNode, 'agdTOCRootLayer');
      domClass.add(this.labelNode, 'agdTOCRootLayerLabel');
    ...
    _createServiceLayerNode: function(serviceLayer){
      ...
      if (serviceLayer._subLayerInfos && serviceLayer._subLayerInfos.length>0) {// group layer
        domClass.add(this.rowNode, 'agdTOCGroupLayer');
        domClass.add(this.labelNode, 'agdTOCGroupLayerLabel');
        this._createChildrenNodes(serviceLayer._subLayerInfos, 'serviceLayer');
      } else {
        domClass.add(this.rowNode, 'agdTOCServiceLayer');
        domClass.add(this.labelNode, 'agdTOCServiceLayerLabel');
    ...
    _createLegendNode: function(rendLeg){
      this._noCheckNode = true;
      domConstruct.destroy(this.containerNode);
      domClass.add(this.labelNode, 'agdTOCLegendLabel');
    ...
    _createChildrenNodes: function(chdn, type){
      ...
      for (var i = 0, n = chdn.length; i < n; i++) {
        ...
        params[type] = chd;
        params.data = chd;
        //var node = new agd.dijit._TOCNode(params);
        if (type == 'legend') {
          chd.id = 'legend' + i;
        }
    ...
    _adjustToState: function(){
      ...
      if (outScale) {
        domClass.add(this.domNode, 'agdTOCOutOfScale');
      } else {
        domClass.remove(this.domNode, 'agdTOCOutOfScale');
      }
      if (this.checkNode) {
        if (this.checkNode.set) {
      ...
      if (this._childTOCNodes.length > 0) {
        array.forEach(this._childTOCNodes, function(child){
          if (child != null && child != undefined) {child._adjustToState();}
        });
      }
    },
  ...
  var _RootLayerTOC = declare([_Widget], {
    ...
    _createRootLayerTOC: function(){
      ...
      ///this._rootLayerNode = new agd.dijit._TOCNode({
  ...
  var TOC = declare("agd.dijit.TOC", [_Widget, Evented], {
    indentSize: 18,
    swatchSize: [30, 30],
    refreshDelay: 500,
    layerInfos: null,
    ...
    _createTOC: function(){
      ...
      for (var i = 0, c = this.layerInfos.length; i < c; i++) {
        ...
        ///var rootLayerTOC = new agd.dijit._RootLayerTOC({
